...
const renderCartPage = (cartItems: unknown[] = []) => {
...
describe('CartPage', () => {
  ...
  describe('数量変更', () => {
    it('+ボタンで数量が増加し合計が再計算される', async () => {
      ...
      renderCartPage([{ productId: 'p1', name: 'Tシャツ', price: 1000, quantity: 1, image: '/img/t.png', stock: 5 }])
    ...
    it('−ボタンで数量が減少し合計が再計算される', async () => {
      const user = userEvent.setup()
      renderCartPage([{ productId: 'p1', name: 'Tシャツ', price: 1000, quantity: 3, image: '/img/t.png', stock: 5 }])
      await user.click(screen.getByText('−'))
      expect(screen.getByDisplayValue('2')).toBeInTheDocument()
      expect(screen.getAllByText('¥2,000')).toHaveLength(2)
    })
