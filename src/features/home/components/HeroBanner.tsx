    ...
    <section className="max-w-7xl mx-auto px-4 mt-6">
      <div className="relative bg-gradient-to-r from-gradient-from to-gradient-to rounded-2xl overflow-hidden h-64 flex items-center">
        <div className="px-10 text-white">
          ...
          <button className="mt-4 bg-white text-brand-700 font-semibold px-6 py-2 rounded-lg hover:bg-gray-100">
            詳しく見る →
          </button>
        </div>
      </div>
      <div className="flex justify-center gap-2 mt-3">
        {banners.map((_, i) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${i === current ? 'bg-brand-600' : 'bg-gray-300'}`}
          />
        ))}
      </div>
    </section>
