...
import ShopsApi from "../api/items/ShopsApi"
import CategoriesApi from "../api/items/CategoriesApi"
...
const ItemService = async (id: number) => { 
    const itemInfo: Item | CommonError = await ItemsApi.fetchItemInfo(id);
    ...
    const shopInfo: Shop | undefined = await ShopsApi.fetchShopInfo(shopId);
    const categories: Categories | undefined = await CategoriesApi.fetchCategoryInfo(categoryId);  
